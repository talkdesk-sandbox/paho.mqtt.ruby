...
module PahoMqtt
  class ConnectionWSHelper
    ...
    def config_socket
      ...
      begin
        web_socket_key = Digest::SHA1.base64digest([
          SecureRandom.base64,
        ...
        headers_handshake = headers.to_a.map { |h| "#{h[0]}: #{h[1]}" }.join("\r\n")
...
CONTENT
        ...
        ws_socket = OpenSSL::SSL::SSLSocket.new(tcp_socket, ssl_context)
        ws_socket.sync_close = true
        ws_socket.connect

        ws_socket.puts(content)

        while line = ws_socket.gets
          if line == "\r\n"
            @socket = ws_socket
            break
          end
